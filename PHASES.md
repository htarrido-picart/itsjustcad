...
- [ ] **M-enviro** — MOSTLY DONE (audited 2026-09-02): `shadowstudy`, `sunhours` grid heatmap, `facesunhours` per-face insolation, EPW import (sets location + reports stats), and **`sunpath` shipped this session** — yearly sun-path dome (7 date arcs Dec→Jun + hour analemmas + horizon ring, golden 'sunpath' layer, undo/replay-stable, deck-callable). **`radiation` also shipped this session** — annual kWh/m²·yr per face from the EPW's DNI/DHI columns (month×hour bins, occlusion-tested beam + isotropic-sky diffuse, bins embedded in the op-log so replay never needs the EPW). Remaining: deck critique hooks for analysis meshes (deck can already read the min/avg/max report lines).
