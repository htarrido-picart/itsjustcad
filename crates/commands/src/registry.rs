...
pub fn registry() -> &'static [CommandSpec] {
    &[
        ...
        CommandSpec {
            ...
            summary: "Generate a road network and blocks from the selected closed site boundary curve (M-intemfit site planning, Phase 5). pattern picks one of four rectilinear generators: orthogonal (recursive minimum-area-OBB split of the site into a grid; a spine road along each split), skewed (the same grid rotated 22.5° into a diagonal layout), organic (a gently sinusoidal spine down the site long axis plus perpendicular connectors), culdesac (a spine with perpendicular stubs ending in turnaround bulbs, spaced by block depth). Roads bake as centerlines onto the 'roads' layer; blocks bake as outlines onto the 'blocks' layer, every block edge tagged with the street it fronts (id/width/length) so a later `lotsubdivide` on a block honours force_street_access. roadwidth = road right-of-way width (curb to curb), blockdepth = block-depth road spacing, alleys=on inserts a narrower rear-lane tier bisecting each block along its long axis, seed makes the layout deterministic (replay-stable). Sticky defaults come from `lotsettings`. radial/hexagonal/voronoi are Phase 5b and not yet built. Example: lotgeneratesite last orthogonal roadwidth=12 blockdepth=60 · lotgeneratesite culdesac blockdepth=50 alleys=on seed=7",
            category: Category::Tools,
        },
        CommandSpec {
            name: "lotloading",
